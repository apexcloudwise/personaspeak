...
import biz.pixelperfectstudios.personaspeak.floris.FlorisPersonaSpeakRow
...
fun TextInputLayout(
    ...
    CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Ltr) {
        Column(
            ...
            Smartbar()
            // PersonaSpeak: dedicated rewrite row between the Smartbar and
            // the keys (ADR-0007 dedicated-row contract). Rendered only when
            // the hosting service exposes the PersonaSpeak host glue.
            (context as? FlorisImeService)?.personaspeakHost?.let { florisHost ->
                FlorisPersonaSpeakRow(florisHost)
            }
