  ...
  public void apply(Project project) {
    ...
    if (project.file("dictionary").exists()) {
        ...
        for (File aospFile : aospFiles) {
          TaskProvider<GenerateWordsListFromAOSPTask> aosp =
              project
                  .getTasks()
                  .register(
                      "parseAospDictionary_" + aospFile.getName().replace('.', '_'),
                      GenerateWordsListFromAOSPTask.class,
                      task -> {
                        task.setInputFile(aospFile);
                        task.setOutputWordsListFile(
                            new File(dictionaryOutputDir, aospFile.getName() + ".xml"));
                        task.setMaxWordsInList(300000);
                      });
          ...
          mergingTask.configure(
              task -> {
                ...
                task.setInputWordsListFiles(
                    arrayPlus(task.getInputWordsListFiles(), aosp.get().getOutputWordsListFile()));
              });
        }
      }
      ...
      if (project.file("dictionary/prebuilt").exists()) {
        File[] prebuiltFiles =
            project.file("dictionary/prebuilt").listFiles((dir, name) -> name.endsWith(".xml"));
      ...
      if (getExtValue(project, "dictionaryTextInputsEnabled", true)
                ...
                .register(
                    ...
                    task -> {
                      ...
                      char[] dictionaryInputPossibleCharacters =
                          getExtValue(project, "dictionaryInputPossibleCharacters", null);
                      if (dictionaryInputPossibleCharacters != null) {
                        ...
                        System.out.println(
                            "Overriding input-text files possible characters to "
                                + new String(dictionaryInputPossibleCharacters));
                      }
                      char[] dictionaryInputAdditionalInnerCharacters =
                          getExtValue(project, "dictionaryInputAdditionalInnerCharacters", null);
                      if (dictionaryInputAdditionalInnerCharacters != null) {
                        ...
                        System.out.println(
                            "Overriding input-text files possible additional inner characters to "
                                + new String(dictionaryInputAdditionalInnerCharacters));
                      }
                    });
        ...
        mergingTask.configure(
            task -> {
              ...
              task.setInputWordsListFiles(
                  arrayPlus(task.getInputWordsListFiles(), inputs.get().getOutputWordsListFile()));
            });
      }
    }
