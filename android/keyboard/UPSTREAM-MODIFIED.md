...
- ime/app/build.gradle — askSourceRoot rewrite of the `gradle/apk_module.gradle` apply, plus PersonaSpeak branding: `override_app_id = 'biz.pixelperfectstudios.personaspeak'`, Kotlin Android + Compose compiler plugins with `buildFeatures.compose`, `minSdkVersion 26` override (ASK-wide minimum stays 23), `implementation project(':personaspeak-ui')`, and Compose runtime dependencies (`platform(libs.compose.bom)`, `libs.compose.ui`). Behavioral delta: the assembled APK is branded PersonaSpeak with SDK-26 floor and links the first-party UI layer with Compose runtime; upstream namespace `com.menny.android.anysoftkeyboard` is unchanged.
