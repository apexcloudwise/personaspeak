...
import com.personaspeak.engine.KeyCodes;
...
/**
 * Ported from ASK's dictionaries.SuggestImpl (Apache-2.0) with two
 * engine-scoped removals: the Context convenience constructor (hosts
 * build the provider) and the quick-text tag search branch (not
 * prediction-engine core; WordComposer's tags state is ignored here).
 * The scoring, callbacks, sub-word splitting, and next-word merging are
 * byte-faithful to the ASK algorithm.
 */
public class SuggestImpl implements Suggest {
  ...
  private String mTypedOriginalWord = "";
  ...
  public SuggestImpl(SuggestionsProvider provider) {
  ...
  public List<CharSequence> getNextSuggestions(
    ...
    if (isValidWord(previousWord)) {
      ...
      Log.d(TAG, "getNextSuggestions for '%s' is invalid.", previousWord);
    }
  ...
  private static class SubWordSuggestionCallback implements Dictionary.WordCallback {
    ...
    private CharSequence mCurrentSubWord = "";
    ...
    void performSubWordsMatching(
        WordComposer wordComposer, SuggestionsProvider suggestionsProvider) {
      ...
      for (var split : splits) {
        ...
        for (var subWord : split) {
          ...
          if (mCurrentBestSubWordSubWordAdjustedFrequency == 0) {
            Log.d(TAG, "Did not find a match for sub-word '%s'", mCurrentSubWord);
            wordCount = -1;
            break;
          }
          ...
          if (currentSplitLength > 0) {
            ...
            mCurrentMatchedWords[currentSplitLength] = (char) KeyCodes.SPACE;
            currentSplitLength++;
          }
          System.arraycopy(
