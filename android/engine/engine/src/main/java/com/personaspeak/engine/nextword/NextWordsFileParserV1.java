...
public class NextWordsFileParserV1 implements NextWordsFileParser {
  ...
  @Override
  public Iterable<NextWordsContainer> loadStoredNextWords(InputStream inputStream)
      throws IOException {
  ...
  private String readWord(byte[] buffer, InputStream inputStream)
      throws IOException {
    ...
    if (bytesToRead == actualReadBytes) {
      return new String(buffer, 0, bytesToRead, EngineCharsets.UTF8);
    } else {
      return null;
    }
  }
  ...
  public void storeNextWords(
      Iterable<NextWordsContainer> nextWords, OutputStream outputStream)
      throws IOException {
