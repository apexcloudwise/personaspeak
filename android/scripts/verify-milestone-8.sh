...
# - Fail-closed active composition & release identity test execution (:ime:app:testDebugUnitTest)
# - Milestone 8 build & signing receipt, R8 report, and plan invariants
#
...
# --- 2. Active composition & release identity unit suite --------------------
echo "[2/3] running release active composition test suite..."
if [ -f "$root/gradlew" ]; then
    run_checked "ReleaseActiveCompositionTest" "$root/gradlew" -p "$root" \
...
# --- 3. Milestone 8 Receipt, R8 Report & Plan Invariants --------------------
echo "[3/3] verifying Milestone 8 evidence receipt, R8 report, and plan invariants..."
...
required_verdicts = [
    'version_identity', 'signing_configuration', 'fail_closed_active_composition',
    'r8_minification_pass', 'dependency_lock_reproducibility', 'slice_b_gating_bound'
]
