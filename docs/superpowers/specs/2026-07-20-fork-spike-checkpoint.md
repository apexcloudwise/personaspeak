...
## UX went first — and is now done

**Completed 2026-07-20**, same session. See
[`2026-07-20-keyboard-ux-design.md`](2026-07-20-keyboard-ux-design.md) and
`docs/design/mockups/`.
...
Onboarding was NOT base-agnostic and was regenerated after the fork decision:
it now pitches replacing the user's keyboard, not adding a second one.

This also sharpens the spike. The graft step above says "four chips,
deliberately ugly" — **that is now superseded**. The real strip is designed
(one persona chip + mood chip + send), so each graft should host the real UI,
and the spike measures the honest question: not "can this codebase host a row
of buttons" but "can this codebase host *our* UI."

One question the spike must now also answer: the base's **own suggestion row**
competes with our strip for vertical space (visible in
`docs/design/mockups/07-strip-variant-a-CHOSEN.png`). Record how much room each
candidate's suggestion strip takes and how hard it is to merge with or replace.
...
Everything below `docs/` on branch `docs/fork-spike-and-ux-brainstorm` is
pushed. Nothing is running; no worker was ever started; no candidate has been
cloned. `main` is untouched.

**Next action: run the spike.** It is the blocking dependency — the base
decides the licence, and the implementation plan's shape depends on whether we
graft onto Kotlin+C++ under GPL-3.0 or Java under Apache-2.0.

1. **Run the three grafts** (§"The spike: which base" above), hosting the real
   strip from the UX design rather than the superseded four-chip placeholder.
   Split per darkmill's ADR-0016: `oc-bg -m zai-coding-plan/glm-5.2` workers
   clone/build/graft and report diffstat + build log; Claude takes the emulator
   serially to install, drive, capture and grade. Workers never grade their own
   screenshots.
2. **Decide the base and the licence together.** If HeliBoard wins, the README's
   Apache-2.0 promise has to change in the same PR.
3. **Write the ADR** superseding ADR-0001, with the spike evidence attached.
   `docs/adr/0001-thin-ime-over-keyboard-fork.md` still reads as Accepted and
   must be marked superseded.
4. **Then** `writing-plans` for the implementation.

Also outstanding, independent of the spike and small enough to do any time:
- Android CI jobs are still commented out in `.github/workflows/ci.yml`, so the
  golden tests are not enforced on PRs.
- No golden-fixture generator exists; `docs/persona-schema.md` claims fixtures
  are "generated by the Python reference" but `personaspeak.py` has no flag
  that prints a prompt.
- `AGENTS.md` module law and `ROADMAP.md` Phase 1 still describe the thin IME.
